...
def gen_files():
    ...
    with open(tempfile) as tf:
        for item in tf:
            if 'True' in item:
                users[re.sub(pattern, r'\2', item).strip('\n')] += 1
                popular_challenges[re.sub(pattern, r'\1', item).strip('\n')] += 1
...
def diehard_pybites():
    ...
    challenge = max(popular_challenges, key=popular_challenges.get)
    challenge_count = popular_challenges[challenge]
    return Stats(user, (challenge, challenge_count))
